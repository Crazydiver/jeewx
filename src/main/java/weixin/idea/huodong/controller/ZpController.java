...
public class ZpController {
	...
	public ModelAndView goGglNew(HttpServletRequest request) {
		...
		if (huodongEntity != null) {
			...
			if (randomNum == 1 || randomNum == 2 || randomNum == 3) {
				String hql = "from PrizeRecordEntity h WHERE h.accountid='"
						+ accountid + "' AND h.prize=" + randomNum
						+ " AND h.hdid='" + huodongEntity.getId() + "'";
				List<PrizeRecordEntity> prizeEntity = this.systemService
						.findByQueryString(hql);
				if (randomNum == 1
						&& prizeEntity.size() >= huodongEntity.getOnetotal()) {
					randomNum = 0;
				} else if (randomNum == 2
						&& prizeEntity.size() >= huodongEntity.getTwototal()) {
					randomNum = 0;
				} else if (randomNum == 3
						&& prizeEntity.size() >= huodongEntity.getThreetotal()) {
					randomNum = 0;
				}
			}
	...
	public AjaxJson doGgl(HttpServletRequest request) {
		...
		List<HdRecordEntity> hdRecrdList = this.systemService
				.findHql(hql, null);
		if (hdRecrdList.size() > 0) {
			...
			HuodongEntity huodongEntitiy = this.systemService.getEntity(
					HuodongEntity.class, hdId);
			if (total < Integer.parseInt(huodongEntitiy.getCount())) {
				...
				List<PrizeRecordEntity> prizeList = this.systemService
	...
	public ModelAndView goZhuanpan(HttpServletRequest request) {
		...
		org.jeecgframework.core.util.LogUtil.info("....hdid...."
				+ hdEntity.getId() + "...openId.." + openId);
	...
	public AjaxJson doZpPize(HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(hdId) && StringUtil.isNotEmpty(openId)) {
			HuodongEntity hdEntity = this.systemService.getEntity(
					HuodongEntity.class, hdId);
			...
			List<HdRecordEntity> hdRecrdList = this.systemService.findHql(hql,
					null);
			Timestamp nowTime = new Timestamp(new java.util.Date().getTime());
			if (hdEntity != null) {
				...
				if (randomNum == 1 || randomNum == 2 || randomNum == 3) {
					// 判读奖品是否已经被抽完
					String hql2 = "from PrizeRecordEntity h WHERE h.accountid='"
							+ accountid
							+ "' AND h.prize="
							+ randomNum
							+ " AND h.hdid='" + hdEntity.getId() + "'";
					List<PrizeRecordEntity> prizeEntity = this.systemService
							.findByQueryString(hql2);
					if (randomNum == 1
							&& prizeEntity.size() >= hdEntity.getOnetotal()) {
						randomNum = 0;
					} else if (randomNum == 2
							&& prizeEntity.size() >= hdEntity.getTwototal()) {
						randomNum = 0;
					} else if (randomNum == 3
							&& prizeEntity.size() >= hdEntity.getThreetotal()) {
						randomNum = 0;
					}
